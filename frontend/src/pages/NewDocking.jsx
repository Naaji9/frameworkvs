...
// (optional) keep single‑file refs for NGL preview:
    ...
    const [plipAnalyzeTopHitsOnly, setPlipAnalyzeTopHitsOnly] = useState(false);
...
const buildFormData = () => {
  ...
  if (enablePlip) {
    ...
    formData.append("plip_analyze_top_hits_only", plipAnalyzeTopHitsOnly ? "true" : "false"); 
  } else {
    ...
    formData.append("plip_analyze_top_hits_only", "false");
  }
...
const handleRun = async () => {
  ...
  try {
    ...
    const genRes = await axios.post(
      "https://backend-strzdw.fly.dev/generate-script-text",
      genForm
    );
...
<div className="mb-8 p-6 bg-gradient-to-r from-purple-50 to-blue-50 rounded-lg border-2 border-purple-200">
  ...
  {enablePlip && (
    <div className="space-y-4 ml-6 p-4 bg-white rounded-md border border-purple-100">
      {/* 🆕 NEW: Analyze Top Hits Only Checkbox */}
      <div className="p-3 bg-blue-50 border border-blue-200 rounded-md">
        <label className="flex items-start text-gray-700 font-medium">
          <input
            type="checkbox"
            className="mr-2 mt-1 w-4 h-4"
            checked={plipAnalyzeTopHitsOnly}
            onChange={(e) => setPlipAnalyzeTopHitsOnly(e.target.checked)}
          />
          <div>
            <span className="block"> Analyze Only Top Hits from Docking Results</span>
            <p className="text-xs text-gray-600 mt-1 font-normal">
              PLIP will only analyze the top-scoring ligands listed in <code className="bg-gray-200 px-1 rounded">top_hits.csv</code> generated by docking.
              This saves time by skipping low-scoring compounds. If you want to analyze all docked results, leave this option unchecked.
            </p>
          </div>
        </label>
      </div>
